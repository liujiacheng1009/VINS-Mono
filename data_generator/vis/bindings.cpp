...
PYBIND11_MODULE(vins_sim_data, m)
{
    ...
    py::class_<DataGenerator>(m, "DataGenerator")
        ...
        .def("get_observed_landmark_ids", [](const DataGenerator &self) { return self.output_gr_ids; },
             "Per-camera slot: landmark pool indices visible in last getImage().")
        .def("get_new_observed_landmark_ids", [](const DataGenerator &self) { return self.output_new_gr_ids; })
