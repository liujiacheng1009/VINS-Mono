...
{
    const int idx = std::min(frame_count, WINDOW_SIZE);
    Ps[idx] = P;
    Rs[idx] = R;
    Vs[idx] = V;
    Bas[idx].setZero();
...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)
    {
        pre_integrations[frame_count]->process(dt, linear_acceleration, angular_velocity);
        //if(solver_flag != NON_LINEAR)
            tmp_pre_integration->process(dt, linear_acceleration, angular_velocity);
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const SimpleHeader &header)
{
...
void Estimator::optimization()
{
    ...
    {
        int j = i + 1;
        if (pre_integrations[j]->sum_dt <= 1e-6 || pre_integrations[j]->sum_dt > 10.0)
            continue;
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            if (ESTIMATE_TD)
            {
                ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j, it_per_id.feature_per_frame[0].velocity, it_per_frame.velocity,
                                                           it_per_id.feature_per_frame[0].cur_td, it_per_frame.cur_td,
                                                           it_per_id.feature_per_frame[0].uv.y(), it_per_frame.uv.y());
                problem.AddResidualBlock(f, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_Feature[feature_index], para_Td[0]);
            }
            else
            {
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        {
            if (pre_integrations[1]->sum_dt > 1e-6 && pre_integrations[1]->sum_dt < 10.0)
            {
    ...
    else
    {
        if (frame_count == WINDOW_SIZE)
        {
            for (unsigned int i = 0; i < dt_buf[frame_count].size(); i++)
            {
                ...
                pre_integrations[frame_count - 1]->process(tmp_dt, tmp_linear_acceleration, tmp_angular_velocity);
